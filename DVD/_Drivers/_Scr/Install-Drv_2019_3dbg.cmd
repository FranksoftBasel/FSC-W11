...
SET V03=SystemProductName
FOR /f "tokens=3*" %%a in ('reg query "HKLM\HARDWARE\DESCRIPTION\System\BIOS" /V "%V03%" ^|findstr /ri "REG_SZ"') do SET HPProductName2=%%a %%b
SET HPProductName2Full=%HPProductName2%
SET HPProductName2=%HPProductName2:-=%
SET HPProductName2=%HPProductName2: =%

@echo.   %%HPProductName2%% %HPProductName2%
...
IF "%HPProductName2%" == "HPProBook6540b" GOTO HPProBook6540b
IF "%HPProductName2%" == "HPProBook6560b" GOTO HPProBook6560b

GOTO EndOfDrvInst

:HPProBook6560b
SET NAME=ProBook 6560b
SET DrvPath=%USB-Stick-Path%\Drivers\HP\%Name%
SET MSG=Install %HPProductNameFull% drivers please wait..
@Echo. 
@Echo.   :HPProBook6560b
@Echo.   PC Vendor ..: %PC-VENDOR%
@Echo.   PC Model ...: %HPProductName2Full%
...
IF EXIST "%DrvPath%\CAMERA\Setup.bat" Start /wait "" cmd /c "%DrvPath%\CAMERA\Setup.bat"
...
CMD /C Shutdown -r -f -t 60 -c "Franksoft Driver Reboot in t-55"
@Timeout 60