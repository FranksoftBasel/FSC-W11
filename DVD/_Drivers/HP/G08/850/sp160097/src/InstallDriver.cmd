...
echo *%windir%\system32\pnputil.exe /add-driver ".\ICLS\*.inf" /install >> "%DRV_Log%"
pnputil.exe /add-driver ".\ICLS\*.inf" /install >> "%DRV_Log%" 2>&1
if %errcode% equ 0 set errcode=%errorlevel%
if %errcode% equ 259 set errcode=0
...
rem echo *%windir%\system32\pnputil.exe /add-driver ".\WMIProvider\*.inf" /install >> "%DRV_Log%"
rem pnputil.exe /add-driver ".\WMIProvider\*.inf" /install >> "%DRV_Log%" 2>&1
