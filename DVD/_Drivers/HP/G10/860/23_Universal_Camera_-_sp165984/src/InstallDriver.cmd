...
rem if not exist %NOINFFLG% if exist "c:\system.sav\tweaks" if exist "c:\system.sav\flags\Proteus.FLG" (
rem     echo ***INFO*** For Preinstall, we've already injected the driver offline --^> skip the installation here. >> "%DRV_Log%"
rem     goto lbl_CommonOps 
rem )
...
rem
%windir%\system32\pnputil.exe /add-driver ".\*.inf" /subdirs /install >> "%DRV_Log%" 2>&1
...
:copy /y ".\Sunplus\Driver\x64\SpITDevMft64.dll" "C:\Windows\System32\" >> "%DRV_Log%" 2>&1
 
xcopy /y ".\SigFile.exe" "C:\Windows\System32\drivers\" >> "%DRV_Log%" 2>&1
:IF EXIST %WINDIR%\System32\DriverStore\FileRepository\ (
:copy /y ".\SigFile.exe" "C:\Windows\System32\DriverStore\FileRepository\" >> "%DRV_Log%" 2>&1
:  ) ELSE (
:copy /y ".\SigFile.exe" "C:\Windows\System32\DriverStore\FileRepository\" >> "%DRV_Log%" 2>&1
:   )
